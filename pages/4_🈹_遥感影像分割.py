...
with left:  
    map = folium.Map(location=[27.96, 120.6], zoom_start=14, tiles=None)    
    folium.TileLayer(
        tiles=
    ...
    folium.TileLayer(
        ...
        overlay=True,
        control=True).add_to(map)
    tif_url = 'http://minio:9000/vi-test/ouhai18cog.tif'
    ...
    if "show" in st.session_state:
        if st.session_state.show:
            ...
            b = g.get_bounds()
            map.fit_bounds(b)
            st.session_state['bounds'] = b        

    folium.LayerControl(position='bottomleft').add_to(map)    
    output = st_folium(map, width=1100, height=750, key='map1', returned_objects=["last_active_drawing"])
...
with right:
    ...
    if btn1:
        ...
        # 切割成块
        id = segutil.split_tif(tif_url, dest_dir, polygon)
